...
# Keep personal checkout and scratch paths out of distributed binaries.
PRIVACY_FLAGS=(
    -Xswiftc -file-prefix-map -Xswiftc "$PROJECT_ROOT=."
    -Xswiftc -debug-prefix-map -Xswiftc "$PROJECT_ROOT=."
    -Xswiftc -file-prefix-map -Xswiftc "$TASK_BUILD_ROOT=.build"
    -Xswiftc -debug-prefix-map -Xswiftc "$TASK_BUILD_ROOT=.build"
    -Xswiftc -file-compilation-dir -Xswiftc .
    -Xcc "-ffile-prefix-map=$PROJECT_ROOT=."
    -Xcc "-fdebug-prefix-map=$PROJECT_ROOT=."
    -Xcc "-ffile-prefix-map=$TASK_BUILD_ROOT=.build"
    -Xcc "-fdebug-prefix-map=$TASK_BUILD_ROOT=.build"
)
swift build --configuration "$TASK_CONFIGURATION" --scratch-path "$TASK_BUILD_ROOT/swift" --disable-sandbox "${PRIVACY_FLAGS[@]}"
...
# Remove debug-only symbols before signing, then fail closed on personal paths.
/usr/bin/strip -S "$APP_BUNDLE/Contents/MacOS/$APP_NAME"
/usr/bin/strings "$APP_BUNDLE/Contents/MacOS/$APP_NAME" > "$TASK_BUILD_ROOT/release-strings.txt"
if /usr/bin/grep -Eq '/Users/|/home/' "$TASK_BUILD_ROOT/release-strings.txt"; then
    printf 'Privacy check failed: personal paths remain in the app binary.\n' >&2
    exit 1
fi
